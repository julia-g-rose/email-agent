...
async def judge_correctness(scenario: Scenario, answer: str) -> CorrectnessJudgeResponse:
    ...
    system_prompt = dedent(
        ...
        Your task is to decide whether the AI answer is fully correct. Require exact dates,
        identifiers, quantities, named entities, and final-event status when the reference
        provides them. For multi-part questions, every requested part must be present.
        Reject missing relevant details, contradictions, unsupported additions, and answers
        that weaken a definite reference statement into a vague possibility.
        """
    )
    messages = [
    ...
    *,
    ...
    traj = ProjectTrajectory(
        ...
        metadata={
            ...
            "step": step,
            # ART's ProjectTrajectory metadata must be scalars, not a list — join
            # the ground-truth Message-IDs (no commas in Enron IDs) and split in the reward.
            "reference_message_ids": ",".join(scenario.message_ids),
        },
    )
    system_prompt = dedent(
        f"""
...
@weave.op
async def rollout(
    model: art.TrainableModel, email_scenario: EmailScenario, temperature: float = 1.0
) -> ProjectTrajectory:
    """ART rollout: run the agent against the trainable model's inference endpoint.

    Training rollouts sample at `temperature=1.0` (the exploration RL needs for reward
    variance); validation passes `temperature=0.0` for a deterministic, best-behavior
    eval that's comparable to the temp-0 closed-model baseline.
